...
$.onLoad = function() {
	...
	xhr.onload = function() {
		...
		if(mapType=='search') {
			...
			newArray = _.filter(mapData.data, function(num){ return num.employeeCount <= employeesTerm && num.totalFundingRaised <= amountTerm; });
			mapData.data = newArray;						
			if(areaTerm != 'All') {
				newArray = _.where(mapData.data, {metroLocation: areaTerm});
				mapData.data = newArray;
			};
			if(fundingTerm != 'All') {
				newArray = _.where(mapData.data, {fundingSource: fundingTerm});
				mapData.data = newArray;
			};			
			if(industryTerm != 'All') {
				newArray = _.where(mapData.data, {industry: industryTerm});
				mapData.data = newArray;
			};						
		};		
		...
		if(mapData.data.length == 0){
			...
			for (var i = 0; i < mapData.data.length; i++){
				...
				rowTitle = Ti.UI.createLabel({
					font:{fontFamily:'TitilliumText25L-800wt', fontSize:'18dp'},
					color:'#4b595b',
				...
				rowDesc = Ti.UI.createLabel({
					font:{fontFamily:'TitilliumText25L-250wt', fontSize:'13dp'},
					color:'#4b595b',
					left:Alloy.Globals.rowRightWidth,
			...
			$.mapView.addEventListener('click', function(evt) {
				    ...
				    if (evt.clicksource == 'leftButton' || evt.clicksource == 'leftPane' ||
				        evt.clicksource == 'leftView' || evt.clicksource == 'rightButton' || evt.clicksource == 'rightPane' ||
				        evt.clicksource == 'rightView' || evt.clicksource == "title" || evt.clicksource == "subtitle") {
...
function showDetail(details){
    ...
    if(industry){
    	var industryLabel = Ti.UI.createLabel({
    		color:'#d15059',
			font:{fontFamily:'TitilliumText25L-800wt', fontSize:'12dp'},
    ...
    if(employees || funding || source) {
    	...
    	if(employees) {
    		...
    		var employeeHeader = Ti.UI.createLabel({
    			...
    			width:0,
    			height:Alloy.Globals.barHeadingHeight,
    			font:{fontFamily:'Surf-Regular', fontSize:'12dp'},
    			textAlign:'center'
    		});
    		var employeeInfo = Ti.UI.createLabel({
    			...
    			width:0,
    			height:Alloy.Globals.barHeadingHeight,
    			font:{fontFamily:'SurfLight', fontSize:'20dp'},
    			textAlign:'center'
    		});    		
    		if(layout==1){employeeHeader.width = '100%'; employeeInfo.width = '100%';};
    		if(layout==2){employeeHeader.width = '49.5%'; employeeInfo.width = '49.5%';};
    		if(layout==3){employeeHeader.width = '32.66%'; employeeInfo.width = '32.66%';};
    	...
    	if(funding&&funding!=0) {
    		...
    		if(layout==1){fundingHeader.width = '100%'; fundingInfo.width = '100%';};
    		if(layout==2){fundingHeader.width = '49.5%'; fundingInfo.width = '49.5%'; fundingInfo.left = '1%'; fundingHeader.left = '1%';};
    		if(layout==3){fundingHeader.width = '32.66%'; fundingInfo.width = '32.66%'; fundingInfo.left = '1%'; fundingHeader.left = '1%';};
    	...
    	if(source) {
    		...
    		if(layout==1){sourceHeader.width = '100%'; sourceInfo.width = '100%';};
    		if(layout==2){sourceHeader.width = '49.5%'; sourceInfo.width = '49.5%'; sourceInfo.left = '1%'; sourceHeader.left = '1%';};
    		if(layout==3){sourceHeader.width = '32.66%'; sourceInfo.width = '32.66%'; sourceInfo.left = '1%'; sourceHeader.left = '1%';};
	...
	var pinLabel = Ti.UI.createLabel({
				text: ' ',
				color: '#303e41',
				font: {fontSize:'14dp',fontFamily:"TitilliumText25L-800wt"},
    ...
    var address = Ti.UI.createLabel({
    	...
    	width:Alloy.Globals.detailMapWidth,
    	top:0,
    	font:{fontFamily:'TitilliumText25L-250wt', fontSize:'12dp'},
    	color:'#676f71',
    	backgroundColor:'#fff',
    	textAlign:'center',
    ...
    $.scrollView.add(address);
    	...
    	var directionsLabel = Ti.UI.createLabel({
    		color:'#dbd1c7',
    		font:{fontFamily:'SurfLight', fontSize:'16dp'},
    		height:Alloy.Globals.detailBtHeight,
    		text:'directions'
    	});
    	if(Ti.Platform.osname != 'android'){
    ...
    if(url || crunchbase){
    	...
    	if(url){
	    	...
	    	if(layout==1){webButton.width = Alloy.Globals.detailMapWidth;};
    		if(layout==2){webButton.width = (Alloy.Globals.detailMapWidth / 2) - .5;};	    	
    	...
    	if(crunchbase){
	    	...
	    	if(layout==1){crunchButton.width = Alloy.Globals.detailMapWidth;};
    		if(layout==2){crunchButton.width = (Alloy.Globals.detailMapWidth / 2) - .5; crunchButton.left = '1';};	    		    	
			if(Ti.Platform.osname != 'android'){
    ...
    if(twitter || facebook || linkedin) {
    	...
    	if(linkedin) {
    		...
    		if(layout==1){linkedInButton.width = '100%';};
    		if(layout==2){linkedInButton.width = '49.5%';};
    		if(layout==3){linkedInButton.width = '32.66%';};
    	...
    	if(facebook) {
    		...
    		if(layout==1){facebookButton.width = '100%';};
    		if(layout==2){facebookButton.width = '49.5%'; if(linkedin){ facebookButton.left = '1%'; };};
    		if(layout==3){facebookButton.width = '32.66%'; facebookButton.left = '1%';};
    	...
    	if(twitter) {
    		...
    		if(layout==1){twitterButton.width = '100%';};
    		if(layout==2){twitterButton.width = '49.5%'; twitterButton.left = '1%';};
    		if(layout==3){twitterButton.width = '32.66%'; twitterButton.left = '1%';};
