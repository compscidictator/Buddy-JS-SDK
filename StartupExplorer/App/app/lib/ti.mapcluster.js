...
TiMapCluster.prototype._cluster = function() {
    ...
    while (markers.length > 0) {
        ...
        /* we were comparing to and remove the original from array. */
       	...
       	var size = (cluster.length < 2) ? cluster.length : 2;
    	...
    	if(Ti.Platform.osname=='android'){
    		annotation.image = 'pins/android_'+size+'.png';
    	} else {
...
Ti.API.info(e.latitudeDelta);
	...
	if(e.latitudeDelta < .01) {
		currentDelta = e.latitudeDelta.toFixed(3);
	} else if(e.latitudeDelta < .1) {
		currentDelta = e.latitudeDelta.toFixed(2);			
	} else if(e.latitudeDelta < 1) {
		currentDelta = e.latitudeDelta.toFixed(1);
	};
