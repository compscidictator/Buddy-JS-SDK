...
public class AsyncHttpClient {
     ...
     */
    public AsyncHttpClient(int socketTimeoutValue, int maxConnections, int socketBufferSize) {
        httpContext = new SyncBasicHttpContext(new BasicHttpContext());
        httpClient = getHttpClient(socketTimeoutValue, maxConnections, socketBufferSize);
    ...
    public AsyncHttpClient() {
        httpContext = new SyncBasicHttpContext(new BasicHttpContext());
        httpClient = getHttpClient(socketTimeout, DEFAULT_MAX_CONNECTIONS, DEFAULT_SOCKET_BUFFER_SIZE);
        httpClient.addRequestInterceptor(new HttpRequestInterceptor() {
            public void process(HttpRequest request, HttpContext context) {
                if (!request.containsHeader(HEADER_ACCEPT_ENCODING)) {
                    request.addHeader(HEADER_ACCEPT_ENCODING, ENCODING_GZIP);
    ...
    /**
     * Set up the httpParams and return the HttpClient
     */
    private DefaultHttpClient getHttpClient(int socketTimeoutValue, int maxConnections, int socketBufferSize)
    {
    	if (socketTimeoutValue == -1)
    	{
    		socketTimeoutValue = DEFAULT_SOCKET_TIMEOUT;
    	}
        ...
        ConnManagerParams.setTimeout(httpParams, socketTimeoutValue);
        ConnManagerParams.setMaxConnectionsPerRoute(httpParams, new ConnPerRouteBean(maxConnections));
        ConnManagerParams.setMaxTotalConnections(httpParams, maxConnections);

        HttpConnectionParams.setSoTimeout(httpParams, socketTimeoutValue);
        HttpConnectionParams.setConnectionTimeout(httpParams, socketTimeoutValue);
        ...
        AsyncSSLSocketFactory socketFactory = null;
        try 
        {
             KeyStore trustStore = KeyStore.getInstance(KeyStore.getDefaultType());
             trustStore.load(null, null);
             socketFactory = new AsyncSSLSocketFactory(trustStore);
             socketFactory.setHostnameVerifier(AsyncSSLSocketFactory.ALLOW_ALL_HOSTNAME_VERIFIER);
        }
        catch (Exception e) 
        {   
        }
